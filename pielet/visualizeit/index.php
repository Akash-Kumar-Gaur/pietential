...
if(!$_COOKIE['email']){
    ...
    echo "<script> location.assign(`/?no-email-found(visualize)`)</script>";
    exit;
}
if (!$_COOKIE['userID']) {
    //exit('no userID cookie');
    header("Location: /?no-userID");
    exit;
} else {
    ...
    if (!$paydirt) {
        // exit('file_get_contents("../ids/$userID.json.txt");');
        header("Location: /");
        exit;
    }
    $master = json_decode($paydirt, true);
    if (!$master['Q0response']) {
        // exit('!$master[Q0response]');
        setcookie('dashboard', '', time() + 7776000, "/");
        setcookie('percent', '', time() + 7776000, "/");
        header("Location: /pielet/survey/?Q0-np-$userID");
        exit;
    }
    if ($master['snapshots']) {
        setcookie('dashboard', 'one', time() + 7776000, "/"); //90 days
        // next time user loads home page, redirected to dash
    }
}
?>
<!DOCTYPE html>
