...
<body style="padding:20px;" cz-shortcut-listen="true">
    ...
    <div id="scoreboard" class="m10" style="width:45%;padding:12px; border:1px solid silver; border-radius:6px;">
 ...
 right: 20px;
 top: 0px;">
        <div id="cc" style="width:45%;"><canvas id="myChart" class="chart"></canvas><br>
        </div>

    </div>
    <img src="logo.png" style="width:45%; margin-bottom:10px;display:none;"><br>
    <form id="mainform" enctype="multipart/form-data" action="" onclick="addValues();" method="post">
        ...
        <div id="emailCapture">
            ...
            <br>
            <div class="bt" onclick="getmail()">Start survey</div>
        </div>
        <div id="progressGroupTop"></div>
        <div id="mainForm" style="display:none;">
            <section style="margin:0px 0px 15px 0px;">
            ...
            <div id="progressGroup">
                ...
                <div id="sectionprogressParent" style="border-radius: 2px; width:100%; background: #eee;padding:0px;margin:10px">
...
rgba(  0, 68,  0,1) `;
        ...
        for (var i = 0; i < formQuestions.length; i++) {
            ...
            if (thePartNoSpace !== token){
                out += `${endDiv}<div id="${thePartNoSpace}" style="background:${colorParts[y]}">`;
                var token = thePartNoSpace;
                endDiv = "</div>";
                y++;
            }
            
            out += `<div id="Q${i}" class="parts ${thePartNoSpace}" data-assignedColor="${returnRandomColor()}" ><strong>${formQuestions[i].Subject}</strong>: ${formQuestions[i].Question}<br><span id="Q${i}Cat1"></span><input value="0" type="range" min="0" max="10" name="Q${i}Cat2" onchange="addValues();" placeholder="Rate on a 0 - 10 scale"> <span></span>
                </div>`;
        }
...
//        part1.style.background = part1color;
//        part2.style.background = part2color;
//        part3.style.background = part3color;
//        part4.style.background = part4color;
//        part5.style.background = part5color;
...
document.addEventListener("DOMContentLoaded", function (event) {
        ...
        function saveForm() {
            ...
            for (var i = 0; i < d.length; i++) {
                if (d[i].type.match(/checkbox|radio/ig)) {
                    if (d[i].checked) {
        ...
        function addValues() {
            ...
            for (const property in obj) {
                ...
                if (property.match(/Q[2-9]\-/g) && Number.isInteger(obj[property])) {
                    SA += parseInt(obj[property]) * 1.25;
                }
                if (property.match(/Q11|Q12|Q13|Q14|Q15|Q16|Q17|Q18|Q19|Q20|Q21|Q22|Q23/g) && Number.isInteger(obj[property])) {
                    EC += parseInt(obj[property]) * .769;
                }
                if (property.match(/Q25|Q26|Q27|Q28|Q29|Q30|Q31/g) && Number.isInteger(obj[property])) {
                    LB += parseInt(obj[property]) * 1.42;
                }
                if (property.match(/Q33|Q34|Q35|Q36|Q37|Q38/g) && Number.isInteger(obj[property])) {
                    //33-38
                    SN += parseInt(obj[property]) * 1.66;
                }
                if (property.match(/Q40|Q41|Q42|Q43|Q44|Q45/g) && Number.isInteger(obj[property])) {
                    //40-45
                ...
                // if (SA<1){ SA = 20;  } else {SA = SA*.277}//8 questions 72 points .277x
                // if (EC<1){ EC = 20; }else {EC = EC*.170}//13 questions 117 points .170x
                // if (LB<1){ LB = 20; }else {LB = LB*.317}//7 questions 63 points .317x
                // if (SN<1){ SN = 20; }else {SN = SN*.357}//6 questions 56 points .357x
                // if (PN<1){ PN = 20; }else {PN = PN*.357}//6 questions 56 points .357x

                // var SAp = SA*.277//8 questions 72 points .277x
                // var ECp = EC*.170//13 questions 117 points .170x
                // var LBp = LB*.317//7 questions 63 points .317x
                // var SNp = SN*.357//6 questions 56 points .357x
                // var PNp = PN*.357//6 questions 56 points .357x
            }
        ...
        function updateTextInput() {
            ...
            for (var i = 0; i < t.length; i++) {
                if (t[i].name.match(/Q/ig)) {
                    document.getElementById(t[i].name).innerHTML = t[i].value;
                    document.getElementById(t[i].name).classList.add("hint");
                }
            }
        ...
        function removeChartFromPhone() {
            if (window.innerWidth < 1000) {
                myChart.parentNode.removeChild(myChart);
            }
        }
        removeChartFromPhone();
        progressGroupTop.innerHTML = progressGroup.innerHTML;
